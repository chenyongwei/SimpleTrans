...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    NSString *language = [appleLanguages objectAtIndex:0]; //en
    NSString *locale = [[NSLocale currentLocale] localeIdentifier]; // en_US
    NSRange range = [language rangeOfString:@"zh" options: NSCaseInsensitiveSearch];
    BOOL isChinese = range.location != NSNotFound;

    if (!isChinese) {
        [[SimpleTranslator translator] initWithCultureCode:@"en"];
    }
    return YES;
}
